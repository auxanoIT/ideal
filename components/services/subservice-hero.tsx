...
export function SubserviceHero({ title, description, image, href, copy: overrideCopy, introduction, enquiryUrl = '/contact' }: Props) {
  ...
  return <div className={styles.wrapper}>
    <section className={styles.hero} aria-labelledby="subservice-title">
      <div className={styles.media}><Image src={image.src} alt={image.alt} fill priority sizes="100vw" quality={85} className={styles.image} /></div>
      ...
      <div className={styles.action}>
        ...
        <Link href={enquiryUrl} className={styles.cta}>{suppliedCopy?.cta ?? "Discuss Your Project"} </Link>
      </div>
    </section>
